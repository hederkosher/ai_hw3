...
IDS::SearchResult IDS::search() {
  ...
  if (start.first < 0 || start.second < 0 || goal.first < 0 ||
      goal.second < 0) {
  ...
  for (int depthLimit = 0; depthLimit <= maxDepthLimit; depthLimit++) {
    ...
    std::vector<std::vector<bool>> visited(
        maze_.getHeight(), std::vector<bool>(maze_.getWidth(), false));
    ...
    bool found =
        depthLimitedDFS(start.first, start.second, 0, depthLimit, currentPath,
                        visited, nodesExpandedThisIteration);
...
bool IDS::depthLimitedDFS(int x, int y, int depth, int maxDepth,
  ...
  for (const auto &neighbor : neighbors) {
    ...
    if (depthLimitedDFS(nx, ny, depth + 1, maxDepth, path, visited,
                        nodesExpanded)) {
...
IDS::SearchResult IDS::getLastResult() const { return lastResult_; }
